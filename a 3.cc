...
private:
    ...
    char board_at(int r, int c) const {
    ...
    void order_moves(vector<Move>& moves, bool is_red_turn, int ply, Move* tt_move = nullptr) {
        sort(moves.begin(), moves.end(), [this, is_red_turn, ply, tt_move](const Move& a, const Move& b) {
            ...
            if (a.is_capture && toupper(a.target) != 'W') {
                score_a = 10000 + get_piece_value(a.target) * 10 - get_piece_value(board_at(a.r1, a.c1));
            }
            if (b.is_capture && toupper(b.target) != 'W') {
                score_b = 10000 + get_piece_value(b.target) * 10 - get_piece_value(board_at(b.r1, b.c1));
            }
            ...
            // History heuristic
            if (!a.is_revival && in_bounds(a.r1, a.c1) && in_bounds(a.r2, a.c2)) {
                score_a += history[a.r1][a.c1][a.r2][a.c2];
            }
            if (!b.is_revival && in_bounds(b.r1, b.c1) && in_bounds(b.r2, b.c2)) {
    ...
    int quiescence(int alpha, int beta, bool is_red_turn) {
        ...
        sort(capture_moves.begin(), capture_moves.end(),
             [this](const Move& a, const Move& b) {
                 int a_value = get_piece_value(a.target) - get_piece_value(board_at(a.r1, a.c1));
                 int b_value = get_piece_value(b.target) - get_piece_value(board_at(b.r1, b.c1));
    ...
    int negamax(int depth, int alpha, int beta, bool is_red_turn, int ply) {
        ...
        if (!in_check) {
            ...
            for (const Move& mv : moves) {
                bool wazir_move = !mv.is_revival && toupper(board_at(mv.r1, mv.c1)) == 'W';
        ...
        for (size_t i = 0; i < moves.size(); i++) {
            ...
            if (alpha >= beta) {
                ...
                if (!move.is_revival && in_bounds(move.r1, move.c1) && in_bounds(move.r2, move.c2)) {
        ...
        while (true) {
            ...
            if (ourTurn) {
                ...
                if (line.length() == 4) {
                    ...
                    char dest = board_at(r2, c2);
                    Move opp_move(r1, c1, r2, c2, dest);

                    if (is_valid_move(opp_move, !isRed)) {
